...
Такая же проблема "загрязнения", вдобавок график распределения CG схож с образцом _YTR_. Обработка не дала значимых улучшений:
...
2. график _Per base sequence content_ избавился от варнинга, но значительно не изменился (надо полагать, что для оригинальных данных он переступил порог не так значительно).
